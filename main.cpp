...
char  Month[12][9]={"января","февраля","марта","апреля","мая","июня","июля",
...
/*  messageBox(mfInformation|mfOKButton,"Осталось запусков: %u",CFG.TimeRemaining);
  if ((CFG.TimeRemaining<=0) || (!f) || (CFG.Size!=sizeof(CFG))) {
    messageBox(mfOKButton|mfInformation,"Вы исчерпали время использования программы. Следующий её запуск может оказаться смертельным. Прощайте...");
    fcloseall();
...
TMenuBar *TApp::initMenuBar( TRect r )
{
	...
	*new TSubMenu( "Файл", kbAltF )+
	    ...
	    *new TMenuItem( "Добавить файл", cmnFileAdd,0,hcNoContext,"")+
	...
	*new TSubMenu("Настройки",kbAltS)+
	    ...
	    *new TMenuItem( "Номер листа",cmnSettingsNum,kbAltN,hcNoContext,"Alt+N")+
...
{
  char buf[100];
  TFileDialog* fd=new TFileDialog("*.lst","Присоединить лист",0,fdOpenButton,0);
...
{
  TDialog*WNum=new TDialog(TRect(20,10,60,15),"Номер листа");
  TInputLine*num=new TInputLine(TRect(25,2,32,3),3);
  WNum->insert(new TButton(TRect(32,2,38,4),"OK",cmCancel,0));
  WNum->insert(num);
  WNum->insert(new TLabel(TRect(2,2,25,3),"Введите новый номер:",num));
...
/*		WSettings		*/
WSettings::WSettings(TConfig cfg): TDialog(TRect(15,0,70,23),"Настройки"),
				TWindowInit( &WSettings::initFrame)
{
  ...
  cbIsAbout=new TCheckBoxes(TRect(2,16,50,17),new TSItem("Показывать AboutBox при запуске",0) );
  cbPrintDate=new TCheckBoxes(TRect(2,17,50,18),new TSItem("Печатать на листах дату",0) );
  ...
  insert(new TButton(TRect(19,20,32,22),"Отмена",cmCancel,bfNormal));
  insert(cbIsAbout);
  insert(cbPrintDate);
  ...
  insert(new TLabel(TRect(1,14,28,15),"Начальный номер (2 знака):",iSeed));
  SetState(cfg);
}
...
void WSettings::GetState(TConfig&cfg)
{
  ...
  strcpy(cfg.OrgName,"АТЦ Кемеровского почтамта т.28-80-64");
...
void WList::evSelectPath(int button)
{
  ...
  if (IsSelect!=PATH) {
    ...
    ArtNum=0;
    if (strlen(dList.Path)>0) {
      FILE*file=fopen(CFG.PathBase,"rb");
      ArtSize=fsize(file)/sizeof(dPath);
      for (int x=0;x<ArtSize;x++) {
	fseek(file,x*sizeof(dPath),SEEK_SET);
	fread(&dPath,sizeof(dPath),1,file);
	if (compare(dPath.Name,dList.Path)) {ArtNum=x;break;}
      }
      fclose(file);
    };
  }
...
void WList::evSelectCar(int button)
{
  if ((IsSelect==CAR)&&(button==0)) {
    ...
    FILE*d=fopen(CFG.DriverBase,"rb");
    fseek(d,sizeof(dDriver)*dCar.Driver,SEEK_SET);
    fread(&dDriver,sizeof(dDriver),1,d);
    fclose(d);
    dList.idDriver=dCar.Driver;
    strcpy(dList.Driver,dDriver.Name);
    strcpy(iDriver->data,dDriver.Name);
  ...
  if (IsSelect!=CAR) {
    ...
    ArtNum=0;
    if (strlen(dList.SAINum)>0) {
      FILE*file=fopen(CFG.CarBase,"rb");
      ArtSize=fsize(file)/sizeof(dCar);
      for (int x=0;x<ArtSize;x++) {
	fseek(file,x*sizeof(dCar),SEEK_SET);
	fread(&dCar,sizeof(dCar),1,file);
	if (compare(dCar.SAINum,dList.SAINum)) {ArtNum=x;break;}
      }
      fclose(file);
    }
  }
...
void WList::evSelectDriver(int button)
{
  ...
  if (IsSelect!=DRIVER) {
    ...
    ArtNum=0;
    if (strlen(dList.Driver)>0) {
      FILE*file=fopen(CFG.DriverBase,"rb");
      ArtSize=fsize(file)/sizeof(dDriver);
      for (int x=0;x<ArtSize;x++) {
	fseek(file,x*sizeof(dDriver),SEEK_SET);
	fread(&dDriver,sizeof(dDriver),1,file);
	if (compare(dDriver.Name,dList.Driver)) {ArtNum=x;break;}
      }
      fclose(file);
    }
  }
				...
				TWindowInit( &WCar::initFrame)
{
  ...
  iDriver=new TInputLine(TRect(15,13,44,14),51);//   <<<--!!!!!!!!!!!!!
  ...
  insert(iType);
  insert(iDriver);
  insert(new TButton(TRect( 45, 13, 56, 15 ), "Найти", cmnSelectDriver, bfNormal));
  ...
  insert(new TLabel(TRect(43,7,45,8),"№",iLicenseNum));
...
void WCar::evFindDriver()
{
  FILE*f=fopen(CFG.DriverBase,"rb");
  int num=0,size=fsize(f)/sizeof(DDriver);
  DDriver driver;
  for (int x=0;x<size;x++) {
    fseek(f,x*sizeof(DDriver),SEEK_SET);
    fread(&driver,sizeof(driver),1,f);
    if (compare(driver.Name,iDriver->data)) {num=x;break;}
  }
  fclose(f);
  data.Driver=num;
  //messageBox(mfOKButton,"found match for '%s': '%s' at %d",iDriver->data,driver.Name,data.Driver);
  strcpy(iDriver->data,driver.Name);
  iDriver->draw();
}
...
void WCar::SetState(DCar src)
{
  ...
  DDriver driver;
  FILE*f=fopen(CFG.DriverBase,"rb");
  fseek(f,src.Driver*sizeof(driver),SEEK_SET);
  fread(&driver,sizeof(driver),1,f);
  strcpy(iDriver->data,driver.Name);
  fclose(f);
  //and it here!
				...
				TWindowInit( &WPath::initFrame)
{
  ...
  insert(new TStaticText(TRect(2,9,17,10),"Время въезда:"));
  insert(new TStaticText(TRect(2,11,17,12),"Время выезда:"));
